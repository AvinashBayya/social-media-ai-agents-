...
import { geminiSummarizeCase, geminiExecutiveBrief, geminiGenerateReport, geminiExtractEntities } from "@/utils/gemini";
...
function AgentsPage() {
  ...
  // Execute REAL Gemini AI analysis
    ...
    try {
      ...
      if (cap === "SUMMARIZE_CASE") {
        ...
        const entitiesText = Array.isArray(res.entities) && res.entities.length > 0
          ...
          : res.raw || "No structured entity data extracted.";
        blocks = [{ heading: "NER Entity Intelligence Extraction (Gemini)", text: entitiesText, monospace: true }];
        ...
        blocks = [{ heading: "Comparative Threat Intelligence (Gemini)", text: res.text }];
        ...
        const res = await geminiGenerateReport({ data: {
        ...
        blocks = [{ heading: `${capLabels[cap] || cap} (Gemini 2.0 Flash)`, text: res.text }];
      }
      ...
      setTerminalLog(prev => [...prev,
        "[LLM] Gemini response received successfully.",
        "[SYS] Compilation complete. Output rendered in intel workspace."
      ]);
      toast.success("Gemini AI Analysis Complete.");
    } catch (err: any) {
      setTerminalLog(prev => [...prev, `[ERROR] ${err?.message || "Gemini API call failed."}`]);
      toast.error("AI analysis failed. Check your GEMINI_API_KEY in .env");
    } finally {
      setLoading(false);
    }
  };
