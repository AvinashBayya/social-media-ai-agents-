...
import { geminiExecutiveBrief } from "@/utils/gemini";
import {
...
const MODULE_CARDS = [
  ...
  { title: "AI Intelligence Assistant", to: "/agents", icon: Bot, color: "text-[#3B82F6]", border: "hover:border-[#3B82F6]/50", desc: "Gemini 2.0 Flash analytical model execution & report compiler" },
];
...
function CommandHub() {
  ...
  const fetchAiBrief = async (targetStr: string) => {
    ...
    try {
      const res = await geminiExecutiveBrief({
        data: {
          target: targetStr,
            ...
            <CardContent className="p-4 space-y-3">
              {loadingBrief ? (
                <div className="space-y-2 py-6 text-center text-xs font-mono text-[#94A3B8]">
                  ...
                  <p>Querying Gemini 2.0 Flash for target intelligence brief...</p>
                </div>
              ) : (
