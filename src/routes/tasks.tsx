...
import { geminiAnalyzeContent, geminiExtractEntities } from "@/utils/gemini";
import {
  ListChecks,
  Shield,
  CheckCircle2,
...
function TasksPage() {
  ...
  // Module 2 real Gemini AI calculation
  const evaluateModule2 = async () => {
    toast.info("Executing Gemini 2.0 Flash NLP analysis...");
    try {
      const [analysisRes, nerRes] = await Promise.all([
        geminiAnalyzeContent({ data: { text: mod2Text } }),
        geminiExtractEntities({ data: { text: mod2Text } })
      ]);
      ...
      toast.success("Module 2 Gemini Analysis Completed.");
    } catch (e: any) {
      toast.error("Gemini NLP analysis failed.");
    }
  };
